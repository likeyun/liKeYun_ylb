    ...
    if(isset($_SESSION["yinliubao"])){
        ...
        if(empty($channel_id) || $channel_id == '' || $channel_id == null || !isset($channel_id)){
            ...
            if($channel_creat_user == $LoginUser){
                ...
                // 用户一致：允许操作
                $delchannel = ['channel_id'=>$channel_id];
                $delchannelResult = $huoma_channel->delete($delchannel);
                
                // 判断操作结果
                if($delchannelResult){
                    
                    // 将当前channel_id的渠道数据也要全部删除
                    // 数据库huoma_channel_data表
                    $huoma_channel_data = $db->set_table('huoma_channel_data');
                    
                    // 操作条件
                    $delchannelData_Bythischannelid = ['channel_id'=>$channel_id];
                    
                    // 执行操作
                    $delchannelData_BythischannelidResult = $huoma_channel_data->delete($delchannelData_Bythischannelid);
                    
                    // 操作结果
                    if($delchannelData_BythischannelidResult){
                        ...
                        // 解析报错信息
                        $errorInfo = json_decode(json_encode($delchannelData_BythischannelidResult,true))[2];
                        if(!$errorInfo){
                    ...
                    // 解析报错信息
                    $errorInfo = json_decode(json_encode($delchannelResult,true))[2];
                    if(!$errorInfo){
