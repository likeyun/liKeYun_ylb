    ...
    if(isset($_SESSION["yinliubao"])){
        ...
        if($conn->query($huoma_dwz) === TRUE){
            ...
            if($conn->query($huoma_dwz_apikey) === TRUE){
                ...
                if($conn->query($huoma_count_count_dwz_pv) === TRUE){
                    ...
                    if(preg_match("/Duplicate column name/", $conn->error)){
                        ...
                        $result = array(
                            'code' => 202,
                            'msg' => '升级失败！请检查数据库是否存在huoma_dwz表、huoma_dwz_apikey表、以及huoma_count表里面是否存在count_dwz_pv字段。如果存在，请删除后再尝试！'
                        );
                    }else{
                        
                        $result = array(
                            'code' => 202,
                            'msg' => '升级失败！huoma_dwz_apikey表创建异常，错误信息：'.$conn->error
                        );
                    }
                }
            }else{
            ...
            if(preg_match("/already exists/", $conn->error)){
                ...
                $result = array(
                    'code' => 202,
                    'msg' => '升级失败！huoma_dwz表创建异常，错误信息：'.$conn->error
                );
            }
        }
