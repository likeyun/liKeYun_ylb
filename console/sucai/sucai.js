...
function getSuCaiList(pageNum) {
    ...
    if(!pageNum){
        ...
        // 如果没有就默认请求第1页
        reqUrl = "../public/getSuCaiList.php?num=30&p=1";
    }else{
        
        // 如果有就请求pageNum的那一页
        reqUrl = "../public/getSuCaiList.php?num=30&p="+pageNum;
    }
        ...
        success: function(res){
            ...
            if(res.code == 200){
                ...
                if(res.totalNum > 20){
                    ...
                    // 渲染分页控件
                    suCaifenyeControl(pageNum,res.nextpage,res.prepage,res.allpage,res.fileType);
...
// 获取文档素材列表
function getDocFilesList(pageNum) {
    
    // 判断是否有pageNum参数传过来
    if(!pageNum){
        
        // 如果没有就默认请求第1页
        reqUrl = "../public/getDocFilesList.php?num=30&p=1";
    }else{
        
        // 如果有就请求pageNum的那一页
        reqUrl = "../public/getDocFilesList.php?num=30&p="+pageNum;
    }
...
// 素材库分页控件
function suCaifenyeControl(thisPage,nextPage,prePage,allPage,fileType){
    ...
    if(thisPage == 1 && allPage == 1){
        ...
        $('#right .data-list .fy').css('width','80px');
        $('#right .data-list .fy').css('margin','0 auto');
        ...
        // 显示控件
        $('#right .data-list .fy').css('display','block');
        $('#right .data-list .fy').css('width','150px');
        $('#right .data-list .fy').css('margin','0 auto');
    }
    ...
    // 设置路由
    window.history.pushState('', '', '?p='+thisPage+'&type='+fileType)
}
...
function delSuCai(){
    ...
    // 获取素材列表
    setTimeout('getSuCaiList(1)',800);
    
    // 弹出提示
    setTimeout('showNotification("删除成功")',1000);
}
