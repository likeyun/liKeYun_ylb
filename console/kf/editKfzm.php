<?php
    ...
    if(isset($_SESSION["yinliubao"])){
    	...
    	$zm_status = trim($_POST['zm_status']);
        ...
        }else if(empty($zm_num) || $zm_num == '' || $zm_num == null || !isset($zm_num)){
            
            $result = array(
			    'code' => 203,
                'msg' => '客服微信号未填写'
		    );
        }else if(empty($zm_status) || $zm_status == '' || $zm_status == null || !isset($zm_status)){
            
            $result = array(
			    'code' => 203,
                'msg' => '二维码状态未设置'
		    );
        }else if(empty($zm_qrcode) || $zm_qrcode == '' || $zm_qrcode == null || !isset($zm_qrcode)){
            
            $result = array(
			    'code' => 203,
                'msg' => '请上传二维码'
		    );
        }else if(empty($zm_id) || $zm_id == '' || $zm_id == null || !isset($zm_id)){
