    ...
    if(isset($_SESSION["yinliubao"])){
        ...
        if(empty($zm_id) || $zm_id == '' || $zm_id == null || !isset($zm_id)){
            ...
            if($kf_creat_user == $LoginUser){
                ...
                // 获取zm_pv
                $zm_pv = json_decode(json_encode($db->set_table('huoma_kf_zima')->find(['zm_id'=>$zm_id])))->zm_pv;
                if($zm_pv == 0){
                    ...
                    // 执行重置
                    // 用户一致：允许操作
                    $resetKfzm = $db->set_table('huoma_kf_zima')->update(['zm_id'=>$zm_id],['zm_yz'=>0,'zm_pv'=>0,'zm_update_time'=>date('Y-m-d H:i:s')]);
                    if($resetKfzm && $resetKfzm == 1){
