...
// 获取封禁名单
function getBanOpenidList(pageNum) {
    
    // 判断是否有pageNum参数传过来
    if(!pageNum){
        
        // 如果没有就默认请求第1页
        reqUrl = "./getBanOpenidList.php";
    }else{
        
        // 如果有就请求pageNum的那一页
        reqUrl = "./getBanOpenidList.php?p=" + pageNum
    }
    ...
    $.ajax({
        ...
        success: function(res){
            ...
            if(res.code == 200){
                ...
                // 遍历数据
                for (var i=0; i<res.banOpenidList.length; i++) {
                    
                    var xuhao = i+1;
                    var kami_id = res.banOpenidList[i].kami_id;
                    var openid = res.banOpenidList[i].openid;
                    var ban_time = res.banOpenidList[i].ban_time;
                    ...
                    var $tbody_HTML = $(
                        ...
                        '       <a href="javascript:;" data-openid="'+openid+'" data-kamiid="'+kami_id+'" onclick="unbanOpenid(this)">解封</a>' +
                        '   </td>' +
                        '</tr>'
                    );
      ...
      error: function(){
        ...
        // 发生错误
        errorPage('data-list','getBanOpenidList.php');
      },
    });
}
...
function unbanOpenid(element){
    ...
    $.ajax({
        type: "POST",
        url: "unbanOpenid.php?openid=" + openId + '&kami_id=' + KamiId,
        success: function(res){
            ...
            if(res.code == 200){
                ...
                // 获取列表
                setTimeout('getBanOpenidList()', 500);
                
                // 显示结果
                setTimeout('showNotification("'+res.msg+'")', 1100);
            }else{
...
// 全部解除
function unbanOpenidAll(){
    
    $.ajax({
        type: "POST",
        url: "unbanOpenidAll.php",
        success: function(res){
            ...
            if(res.code == 200){
                ...
                // 隐藏Modal
                setTimeout("hideModal('unbanOpenidAllModal')", 500);
                
                // 获取列表
                setTimeout('getBanOpenidList()', 700);
                
                // 显示结果
                setTimeout('showNotification("'+res.msg+'")', 1000);
            }else{
        ...
        error: function() {
            ...
            // 服务器发生错误
            showErrorResultForphpfileName('unbanOpenidAll.php');
        }
    });
}
...
// 初始化（getBanOpenidList）
function initialize_getBanOpenidList(){
