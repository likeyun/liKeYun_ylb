    ...
    if(isset($_SESSION["yinliubao"])){
        ...
        if(empty($shareCard_id) || !isset($shareCard_id)){
            ...
            if($getshareCardInfoResult && $getshareCardInfoResult > 0){
                ...
                // 入口域名
                $shareCard_rkym = json_decode(json_encode($getshareCardInfoResult))->shareCard_rkym;
                
                // 生成longUrl
                $longUrl = dirname(dirname(dirname($shareCard_rkym.$_SERVER["REQUEST_URI"]))).'/common/shareCard/redirect/?sid='.$shareCard_id;
