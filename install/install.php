<?php
    ...
    if(empty($db_host) || !isset($db_host)){
        ...
        if($conn->connect_error == 'Connection timed out'){
            ...
            // 设置管理员账号和密码
            $huoma_user_Data = "INSERT INTO `huoma_user` (`user_id`, `user_name`, `user_pass`, `user_expire`, `user_email`, `user_mb_ask`, `user_admin`, `user_manager`, `user_beizhu`, `user_group`) VALUES (100000, '".$user_name."', '".MD5($user_pass)."', '2033-12-31 23:59:59', '".$user_email."','请选择密保问题', 1, '".$user_name."', '超级管理员', '默认')";
            ...
            // 添加默认的域名
            $huoma_domain_Data = "INSERT INTO `huoma_domain` (`domain_id`, `domain_type`, `domain`, `domain_usergroup`) VALUES
            (100000, 1, '".$HTTP_TYPE.$_SERVER['HTTP_HOST']."', '[\"默认\"]'),
            (100001, 2, '".$HTTP_TYPE.$_SERVER['HTTP_HOST']."', '[\"默认\"]'),
            (100002, 3, '".$HTTP_TYPE.$_SERVER['HTTP_HOST']."', '[\"默认\"]'),
            (100003, 4, '".$HTTP_TYPE.$_SERVER['HTTP_HOST']."', '[\"默认\"]')";
            ...
            $ylb_jumpWeChat = "CREATE TABLE `ylb_jumpWeChat` (
              ...
              `jw_yccym` varchar(255) DEFAULT NULL COMMENT '对象存储域名',
            ...
            // 设置用户组为默认
            mysqli_query($con,"UPDATE huoma_user SET Age=36 WHERE FirstName='Peter' AND LastName='Griffin'");
            ...
            if(file_exists('./install.lock') || file_exists('../console/Db.php')){
                ...
                if(
                    ...
                    $conn->query($ylb_jumpWeChat) === TRUE){
