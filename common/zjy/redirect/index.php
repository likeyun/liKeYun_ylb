    ...
    <body>
        ...
        if($zid){
            ...
            if($getZjyInfo){
                ...
                // 获取成功
                $zjy_ldym = json_decode(json_encode($getZjyInfo))->zjy_ldym;
                ...
                if($getDomainNameCheckConfig){
                    ...
                    if($domainCheck_status == 1){
                        ...
                        $checkDomainURL = $httpOrhttps.'://'.$getThisPagePath.'/console/public/domainNameCheck.php?domain='.$zjy_ldym;
                        ...
                        if($checkCode == 200){
                            ...
                            // 不正常
                            // 发送通知
                            sendNotification($domainCheck_channel,'淘宝客中间页'.$zjy_ldym.'域名被封了！尽快处理！',$db);
                            
                            // 是否有备用域名
                            if($domainCheck_byym){
                                
                                // 使用备用域名跳转
                                jump($domainCheck_byym,$zid);
                            }else{
